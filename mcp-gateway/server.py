...
async def _enrich_analysis_with_file_metrics(
    client: httpx.AsyncClient,
    repo_id: str,
    analysis: dict[str, Any],
) -> dict[str, Any]:
    """Ensure analyze results include largest_files and concrete recommendation targets."""
    metrics = analysis.get("metrics") or {}
    if not metrics.get("largest_files"):
        try:
            file_metrics = await _expect_json(
    ...
    if only_generic:
        try:
            rec_payload = await _expect_json(
                await client.post(
    ...
    try:
        ...
        if redsl_res.status_code == 200:
            ...
            if data.get("engine") == "redsl":
                ...
                return await _enrich_analysis_with_file_metrics(client, repo_id, analysis)
    except Exception:
        pass
